...
class Proxy_:
	...
	func callback_(func_name, args := []):
		if not object_to_proxy_:
			var result = load("res://HevLib Discord RPC/lib/discord_result.gdns").new()
...
class LobbyTransaction extends Proxy_:
	func _init(lobby_transaction).(lobby_transaction) -> void:
		pass
...
class Activity extends Proxy_:
	func _init().(load("res://HevLib Discord RPC/lib/discord_activity.gdns").new()) -> void:
		pass
...
class ActivityManager_ extends Proxy_:
	...
	func update_activity(activity:Activity):
...
class LobbyManager_ extends Proxy_:
	...
	func connect_lobby_with_activity_secret(secret:String):
		return callback_("connect_lobby_with_activity_secret", [secret])

	func get_lobby_create_transaction() -> LobbyTransaction:
		var transaction = load("res://HevLib Discord RPC/lib/discord_lobby_transaction.gdns").new()
	...
	func create_lobby(transaction:LobbyTransaction):
		return callback_("create_lobby", [transaction.object_to_proxy_])

	func send_lobby_message(lobby_id:int, message:String):
	...
	func get_member_user(lobby_id:int, user_id:int) -> User:
		var user = load("res://HevLib Discord RPC/lib/discord_user.gdns").new()
...
class OverlayManager_ extends Proxy_:
	...
	func open_activity_invite(activation_type:int):
...
func _ready():
	...
	discore_core_ = load("res://HevLib Discord RPC/lib/discord_core.gdns").new()
	if discore_core_:
